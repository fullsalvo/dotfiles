...
# Font (Configurable with "-F" and "--font" at launch)
# Currently only supported for X Terminals
if [ -f $HOME/.XResources ]; then
    font=$(cat $HOME/.XResources | grep -v ! | awk '/font/ {print $2}' | sed 's/xft://;s/:pixelsize.*$//')
elif [ -f $HOME/.Xresources ]; then
    font=$(cat $HOME/.Xresources | grep -v ! | awk '/font/ {print $2}' | sed 's/xft://;s/:pixelsize.*$//')
elif [ -f $HOME/.XDefaults ]; then
    font=$(cat $HOME/.XDefaults | grep -v ! | awk '/font/ {print $2}' | sed 's/xft://;s/:pixelsize.*$//')
elif [ -f $HOME/.Xdefaults ]; then
    font=$(cat $HOME/.Xdefaults | grep -v ! | awk '/font/ {print $2}' | sed 's/xft://;s/:pixelsize.*$//')
else
    font="Unknown"
...
# Screen Resolution (Configurable with "-R" and "--resolution" at launch)
...
usage () {
    ...
    printf '%s\n' "   --font                 Manually set the font name"
    printf '%s\n' "   --resolution           Manually set the resolution"
...
printinfo "$title_font" "$font"
printinfo "$title_resolution" "$resolution"
